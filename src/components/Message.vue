...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://reverent-leakey-deb3ce.netlify.app/api/like",
              data: {
        ...
        axios
          .post("https://reverent-leakey-deb3ce.netlify.app/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://reverent-leakey-deb3ce.netlify.app/api/shares/" +
    ...
    async getShares() {
      ...
      for (let i = 0; i < shares.data.data.length; i++) {
        await axios
          .get(
            "https://reverent-leakey-deb3ce.netlify.appL/api/shares/" +
